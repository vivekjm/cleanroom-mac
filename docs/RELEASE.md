...
The desktop app is intentionally high-level. It opens without a heavy storage scan, uses the fast dashboard summary for top-level Scan/Review refreshes, keeps implementation details hidden, lets users stop long detailed actions with guarded process tracking, and applies the safe cleanup flow through the native confirmation sheet.
