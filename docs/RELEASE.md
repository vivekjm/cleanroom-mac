...
The desktop app is intentionally high-level. It opens without a heavy storage scan, uses fast summaries for top-level reviews, keeps implementation details hidden, labels cleanup as native previews and confirmations instead of command steps, lets users stop long detailed actions with guarded process tracking, and applies the safe cleanup flow through the native confirmation sheet.
