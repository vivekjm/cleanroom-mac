...
struct AppSanitizerTests {
    static func main() {
        ...
        expect(AppState.visibleReviewItemLimitForTesting(folderAction) == 40, "initial folder reviews should show the requested number of rows")
        expect(AppState.visibleReviewItemLimitForTesting(folderAction.expandedReviewLimitForTesting()) == 120, "show more should let the app display the expanded row set")
        ...
        let veryLargeAction = AppAction(title: "Huge Review", args: ["documents-fast", "/Users/example/Documents/media-project", "--limit", "900"])
        expect(AppState.visibleReviewItemLimitForTesting(veryLargeAction) == 500, "desktop review lists should stay capped for very large folders")
